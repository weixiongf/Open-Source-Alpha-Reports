...
所有报告发布后自动登记到 [list/](list/) 目录，提供两种互补的检索视角：按时间回溯的**月度清单**，与只留头部精华的**高分项目榜**。

### 月度清单 · 全量流水

`list/report_list_YYYY年MM月.md`，按年月归档，逐条记录序号 / 项目 / 综合得分 / 等级 / 一句话结论，适合按时间回溯与横向对比。首期已收录：
...
[**list/高分项目榜_80分以上.md**](list/高分项目榜_80分以上.md)，一个**独立、跨时间**的精华索引，不按年月归档：只收录综合得分 **≥ 80（S 级明星资产）** 的项目，同一项目多次分析**仅保留历史最高分**，并按得分从高到低排列，每条直达对应 PDF 报告。想跳过海量流水、直接锁定「最值得投入」的头部标的，看这一份就够了。

> 榜单在每次报告发布后自动重算、去重与排序，实时排名以清单文件为准。

完整报告见 `reports/` 目录（按日期归档），全部清单见 `list/` 目录。
...
1. 点击右上角 **Star**，持续接收更新
2. 进入 `reports/` 按日期浏览 PDF，或先看 [高分项目榜](list/高分项目榜_80分以上.md) 快速锁定 ≥ 80 分的头部标的
